...
let sameClickIntervals = 0
let lastTimeStampChit = 0
let lastTimeIntervalChit = 0
const chitBlockClicks = 100
let chitBlockCount = 0
function checkChit() {
    const time = Date.now()
    const interval = time - lastTimeStampChit
    console.log(interval)
    if (Math.abs(interval - lastTimeIntervalChit) < 5) sameClickIntervals++
    else sameClickIntervals = 0
    lastTimeStampChit = time
    lastTimeIntervalChit = interval

    if (sameClickIntervals > 4) {
        sameClickIntervals = 0
        lastTimeStampChit = 0
        lastTimeIntervalChit = 0

        chitBlockCount = chitBlockClicks
        alert('CHIT!!!')
    }
}
...
function getClick() {
    if (chitBlockCount > 0) return chitBlockCount--
